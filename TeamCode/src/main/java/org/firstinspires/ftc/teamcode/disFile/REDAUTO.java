...
@Disabled
@Autonomous(name = "🔴 RED-FIX-FIX-FIX ", group = "Autonomous")
@Configurable
public class REDAUTO extends OpMode {
    private Servo pushServo;
    private Servo sorterServo;
    private RevColorSensorV3 colorSensor;
    private DcMotorEx shooter1;  // DcMotorEx for PIDF velocity control
    ...
    // SHOOTER RPM RANGE (PIDF-CONTROLLED) - LOWERED FOR SHOOTING TOO HIGH
    private static final double RPM_MIN = 200;   // Close shots (reduced from 500)
    private static final double RPM_MAX = 1800;  // Far shots (reduced from 1800)
    private double targetShooterRPM = 900;       // Default RPM (reduced from 1500)
    ...
    // DYNAMIC SHOOTER CONTROL: Adjusts both RPM (PIDF) and servo angle based on distance
    private void updateShooterVelocity() {
        ...
        if (result != null && result.isValid()) {
            ...
            // Calculate target RPM using linear interpolation
            ...
            // Apply velocity command - PIDF controller maintains this RPM automatically
            shooter1.setVelocity(targetShooterRPM);

            // Calculate and apply shooter servo angle
            ...
            // Fallback: use default RPM if no Limelight target
            shooter1.setVelocity(targetShooterRPM);
        }
    }

    // Set fixed shooter RPM (PIDF maintains velocity)
    ...
    public void updateStateMachine() {
        switch (pathState) {
            ...
            case PATH_1:
                ...
                if (!follower.isBusy()) {
                    setShooterVelocity(900);  // Reduced from 1500 to 900 RPM
            ...
            case PATH_3:
                if (!follower.isBusy()) {
                    ...
                    resetBallState();
                    intake(0);
                    setShooterVelocity(900);  // Reduced from 1500
    ...
    public void init() {
        try {
            ...
            // ═══════════════════════════════════════════════════════════════
            // PIDF SETUP FOR SHOOTER MOTOR (DcMotorEx)
            // ═══════════════════════════════════════════════════════════════
            shooter1 = hardwareMap.get(DcMotorEx.class, "shooter1");
            shooter1.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
            shooter1.setDirection(DcMotorSimple.Direction.REVERSE);

            // YOUR TUNED PIDF COEFFICIENTS FROM TELEOP
            PIDFCoefficients pidf = new PIDFCoefficients(31, 0, 0, 23);
            shooter1.setPIDFCoefficients(DcMotor.RunMode.RUN_USING_ENCODER, pidf);
            // P=31: Proportional gain for error correction
            // I=0:  No integral needed for flywheel
            // D=0:  No derivative needed
            // F=23: Feedforward - base power for velocity maintenance
            // ═══════════════════════════════════════════════════════════════
            ...
            telemetry.addLine("═══════════════════════════════");
            telemetry.addLine("🎯 PIDF ENABLED");
            telemetry.addLine("   P=31, I=0, D=0, F=23");
            telemetry.addLine("🚀 RPM Range: " + (int)RPM_MIN + "-" + (int)RPM_MAX);
            telemetry.addLine("   (LOWERED - was shooting high)");
    ...
    public void loop() {
        try {
            ...
            if (launchMode) {
                ...
                // CRITICAL: Updates both RPM and servo angle dynamically
