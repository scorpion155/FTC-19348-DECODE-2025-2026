...
@Autonomous(name = "🔴 RED-fixed ", group = "Autonomous")
@Configurable
public class pgp extends OpMode {
    private Servo pushServo;
    private Servo sorterServo;
    private RevColorSensorV3 colorSensor;
    private DcMotorEx shooter1;  // Changed to DcMotorEx for velocity monitoring
    ...
    // NEW: Recovery time constant - adjust this based on testing
    private static final double RECOVERY_TIME = 0.5;  // Time for flywheel to recover between shots
    private int shotsComplete = 0;  // Track number of shots completed

    enum LaunchState {
        IDLE, ROTATE, WAIT_SETTLE, PUSH_UP, WAIT_UP, PUSH_DOWN, WAIT_DOWN, RECOVERY  // Added RECOVERY state
    }
    ...
    public static class Paths {
        ...
        public Paths(Follower follower) {
            ...
            Path1 = follower
                    .pathBuilder()
                    .addPath(
                            new BezierLine(new Pose(59.000, 129.000), new Pose(50.000, 112.000))
                    )
            ...
            Path2 = follower
                    .pathBuilder()
                    .addPath(
                            new BezierLine(new Pose(50.000, 112.000), new Pose(28.000, 112.000))
                    )
                    .setLinearHeadingInterpolation(Math.toRadians(180), Math.toRadians(180))
                    .build();
            ...
            Path4 = follower
                    .pathBuilder()
                    .addPath(
                            new BezierLine(new Pose(58.000, 118.000), new Pose(47.000, 91.000))
                    )
            ...
            Path5 = follower
                    .pathBuilder()
                    .addPath(
                            new BezierLine(new Pose(47.000, 91.000), new Pose(28.000, 91.000))
                    )
                    .setLinearHeadingInterpolation(Math.toRadians(180), Math.toRadians(180))
                    .build();
            ...
            Path6 = follower
                    .pathBuilder()
                    .addPath(
                            new BezierLine(new Pose(28.000, 91.000), new Pose(47.000, 77.000))
                    )
                    .setLinearHeadingInterpolation(Math.toRadians(180), Math.toRadians(180))
                    .build();
            ...
            Path7 = follower
                    .pathBuilder()
                    .addPath(
                            new BezierLine(new Pose(47.000, 77.000), new Pose(28.000, 77.000))
                    )
                    .setLinearHeadingInterpolation(Math.toRadians(180), Math.toRadians(180))
                    .build();
            ...
            Path8 = follower
                    .pathBuilder()
                    .addPath(
                            new BezierLine(new Pose(28.000, 77.000), new Pose(47.000, 77.000))
                    )
                    .setLinearHeadingInterpolation(Math.toRadians(180), Math.toRadians(180))
                    .build();
            ...
            Path9 = follower
                    .pathBuilder()
                    .addPath(
                            new BezierLine(new Pose(47.000, 77.000), new Pose(47.000, 77.000))
                    )
    ...
    private void resetBallState() {
        ...
        servoMoving = false;
        useForceIndex = false;
        shotsComplete = 0;  // Reset shot counter
    ...
    public void updateStateMachine() {
        switch (pathState) {
            ...
            case PATH_1:
                // Initialize aim motor to +95 ticks in PATH_1 (RED ALLIANCE)
                if (!initAimComplete) {
                    aimMotor.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                    aimTargetTicks = 95;  // Flipped from -95 for red alliance
            ...
            case PATH_3:
                if (!follower.isBusy()) {
                    ...
                    follower.setMaxPower(1);
                    path3DelayStarted = false;
                    aim(-45);  // Flipped from +45 for red alliance
                }
                break;
            ...
            case PATH_6:
                if (!follower.isBusy()) {
                    ...
                    driveForward(-0.8);
                    follower.setMaxPower(1);
                    aim(70);  // Flipped from -70 for red alliance
                }
                break;
    ...
    public void init() {
        try {
            ...
            // Changed to DcMotorEx for velocity monitoring
    ...
    public void loop() {
        try {
            ...
            // Enhanced telemetry with shooter velocity
            ...
            telemetry.addData("🎯 Shooter Vel", "%.0f ticks/s", shooter1.getVelocity());
            telemetry.addData("Launch State", launchState);
            telemetry.addData("Shots Done", shotsComplete);
            telemetry.update();
    ...
    public void startLaunch(int balls) {
        ...
        launching = true;
        launchMode = true;
        shotsComplete = 0;  // Reset counter when starting new launch sequence
    }
        ...
        switch (launchState) {
            ...
            case WAIT_DOWN:
                if (launchTimer.seconds() > PUSH_DOWN_TIME) {
                    shotsComplete++;  // Increment shot counter
                    intakeIndex = (intakeIndex + 1) % LAUNCH_POS.length;
                    launchTimer.reset();

                    // NEW: Transition to RECOVERY state instead of IDLE
                    launchState = LaunchState.RECOVERY;
                }
                break;

            // NEW: Recovery state allows flywheel to spin back up
            case RECOVERY:
                if (launchTimer.seconds() > RECOVERY_TIME) {
                    launchState = LaunchState.IDLE;
                }
                break;
        }
