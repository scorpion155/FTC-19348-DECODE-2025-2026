...
public class TurretGoalAimer {
    ...
    public TurretGoalAimer(DcMotor turretMotor, IMU imu, Telemetry telemetry) {
    ...
    public void update(boolean manualNow, double manualPower, Pose robotPose, Pose goalPose) {
        ...
        turretIntegral = Math.max(-INTEGRAL_CLAMP, Math.min(INTEGRAL_CLAMP, turretIntegral));
        ...
        cmdPower = Math.max(-TURRET_MAX_POWER, Math.min(TURRET_MAX_POWER, cmdPower));
    ...
    private void applyManualPower(double manualPower) {
        ...
        requested = Math.max(-1.0, Math.min(1.0, requested));
